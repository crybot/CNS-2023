...
class HyperOptimizer():
  def __init__(self, default_config, sweep_config, plot_metric = False):
    self.best_config = default_config.copy()
  ...
  def _bayesian(self, score_function, parameters, ranges, iterations=30, sample_size=500):
    ...
    for i in range(1, iterations):
      ...
      if self.plot_metric:
          plt.plot(range(i + 1), history)
          plt.plot([best_iteration], [self.best_metric], 'tab:red', marker='o')
          plt.draw()
          plt.pause(0.001)
          plt.clf()
  ...
  def optimize(self, score_function, return_error=False, window_iterations=1):
    ...
    iterations = self.sweep_config['iterations']
